...
async def execute(args: Dict[str, Any], context: Dict[str, Any]) -> str:
    ...
    try:
        ...
        for i, node in enumerate(nodes):
            # 记录第一个节点的结构用于调试
            if i == 0:
                logger.debug(f"合并转发节点示例结构: {json.dumps(node, ensure_ascii=False)[:500]}")

            sender = node.get("sender") or {}
            # 兼容有些实现直接把发送者信息放在节点根部
            sender_name = sender.get("nickname") or node.get("nickname") or sender.get("card") or node.get("card") or "未知用户"
            sender_id = sender.get("user_id") or node.get("user_id") or "未知ID"
            
            node_time = node.get("time")
            if node_time:
                try:
                    timestamp = datetime.fromtimestamp(float(node_time)).strftime("%Y-%m-%d %H:%M:%S")
            ...
            # 激进的内容提取：尝试所有可能的字段
            raw_content = node.get("content") or node.get("message") or node.get("raw_message")
                ...
                for segment in raw_content:
                    ...
                    if seg_type == "text":
                        ...
                        try:
                            ...
                            desc = j_data.get("meta", {}).get("detail", {}).get("desc", "JSON消息")
...
<content>{text}</content>
        ...
        logger.info(f"get_forward_msg 处理完成，返回数据样例 (前500字符): {result[:500]}...")
        return result
