...
class TaskSchedulerConfig(BaseModel):
    """任务调度器配置"""
    enabled: bool = Field(default=True, description="是否启用任务调度器")
    max_steps: int = Field(default=10, ge=1, le=100, description="最大保存步骤数")
    compression_threshold: int = Field(default=5, ge=1, le=50, description="压缩阈值")
    keep_last_steps: int = Field(default=3, ge=1, le=20, description="压缩后保留步骤数")
