...
class ConversationAnalyzer:
    ...
    def _build_prompt(self, messages: List[Dict[str, str]]) -> str:
        ...
        # 过滤掉工具JSON结果，只保留自然语言对话
        filtered_messages = []
        for m in messages:
            content = m.get('content', '')
            # 跳过JSON工具结果（包含"success", "status", "tool_calls"等关键词）
            if any(keyword in content for keyword in ['"success"', '"status"', '"tool"', '"app_name"', '"message"']):
                continue
            filtered_messages.append(m)
        
        # 只使用最新的一条用户消息和一条助手回复
        recent_messages = filtered_messages[-2:] if len(filtered_messages) >= 2 else filtered_messages[-1:]
        ...
        # 调试：打印构建的对话内容
        logger.info(f"[ConversationAnalyzer] 构建对话内容, 原始消息数: {len(messages)}, 过滤后消息数: {len(filtered_messages)}, 使用最近消息数: {len(recent_messages)}")
        logger.info(f"[ConversationAnalyzer] 对话内容:\n{conversation}")
        ...
        try:
            ...
            for name, info in services_info.items():
                if info:
                    ...
                    # 提取工具名称 - invocationCommands 在 manifest 的根级别
                    tools = []
    ...
    def _parse_non_standard_json(self, text: str) -> List[Dict[str, Any]]:
        """解析非标准JSON格式 - 处理中文括号、标准JSON和JSON数组"""
        from nagaagent_core.stable.parsing import parse_non_standard_json
        
        result = parse_non_standard_json(text)
        
        # 如果结果是单个字典，包装成列表（保持向后兼容）
