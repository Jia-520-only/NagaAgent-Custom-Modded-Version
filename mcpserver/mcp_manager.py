...
import logging
import inspect
from typing import Dict, Optional, List, Any, Callable, Awaitable, Generic, TypeVar, Union, cast, TypedDict
    ...
    async def unified_call(self, service_name: str, tool_name: str, args: dict):
        ...
        try:
            ...
            if agent:
                ...
                # 检查是否有call_tool方法（用于某些自定义agent）
                elif hasattr(agent, 'call_tool'):
                    logger.info(f"MCP调用(使用call_tool): {service_name}.{tool_name} with args: {args}")

                    # 调用call_tool方法
                    if asyncio.iscoroutinefunction(agent.call_tool):
                    ...
                    logger.error(f"MCP服务 {service_name} 没有可用的调用方法(handle_handoff 或 call_tool)")
