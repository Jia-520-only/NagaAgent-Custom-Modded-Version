...
class WeatherTimeTool:
    ...
    def _resolve_autonomous_prefecture(self, province: str, city_name: str) -> str:
        """
        解析自治州名称，返回对应的中心城市名称
        ...
        return autonomous_map.get(city_name, city_name)
    ...
    async def handle(self, action=None, ip=None, city=None, query=None, format=None, **kwargs):
        ...
        # 解析自治州，转换为中心城市
        if city_name:
            city_name = self._resolve_autonomous_prefecture(province, city_name)

        # 查询城市代码（移除空格）
        city_key = f'{province}{city_name}'.replace(' ', '')
        city_code = codes_map.get(city_key)

        if not city_code:
            return {'status': 'error', 'message': f'未找到城市编码或该城市不存在: {city_name}, province: {province}, key: {city_key}, 确保city格式为{{省 市}}, 如:湖北 武汉, 直辖市请重复两遍市名, 如:北京 北京'}
