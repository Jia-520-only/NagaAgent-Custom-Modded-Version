...
class SystemConfig(BaseModel):
    ...
    active_communication: bool = Field(default=False, description="是否启用主动交流模式")
    voiceprint_enabled: bool = Field(default=False, description="是否启用声纹识别系统")
    voiceprint_owner_name: str = Field(default="", description="声纹主人姓名")
    diary_enabled: bool = Field(default=False, description="是否启用AI日记功能")
    diary_auto_save: bool = Field(default=True, description="是否自动保存日记")
...
class BrowserConfig(BaseModel):
    ...
    edge_lnk_path: str = Field(
        default="",
        description="Edge浏览器快捷方式路径（留空自动检测）"
    )
    edge_common_paths: List[str] = Field(
        default_factory=lambda: [
            os.path.expanduser(r'~\Desktop\Microsoft Edge.lnk'),  # 用户桌面
...
class TTSConfig(BaseModel):
    ...
    # 多引擎TTS支持
    default_engine: str = Field(default="edge_tts", description="默认TTS引擎: edge_tts, gpt_sovits, vits")

    # GPT-SoVITS配置
    gpt_sovits_enabled: bool = Field(default=False, description="是否启用GPT-SoVITS")
    gpt_sovits_url: str = Field(default="http://127.0.0.1:9880", description="GPT-SoVITS服务地址")
    gpt_sovits_speed: float = Field(default=1.0, ge=0.1, le=3.0, description="GPT-SoVITS语速")
    gpt_sovits_top_k: int = Field(default=15, ge=1, le=50, description="GPT-SoVITS top_k")
    gpt_sovits_top_p: float = Field(default=1.0, ge=0.0, le=1.0, description="GPT-SoVITS top_p")
    gpt_sovits_temperature: float = Field(default=1.0, ge=0.1, le=2.0, description="GPT-SoVITS temperature")
    gpt_sovits_ref_free: bool = Field(default=False, description="GPT-SoVITS是否免参考")
    gpt_sovits_ref_text: str = Field(default="", description="GPT-SoVITS参考文本")
    gpt_sovits_ref_audio_path: str = Field(default="", description="GPT-SoVITS参考音频路径")
    gpt_sovits_filter_brackets: bool = Field(default=False, description="GPT-SoVITS是否过滤括号内容")
    gpt_sovits_filter_special_chars: bool = Field(default=False, description="GPT-SoVITS是否过滤特殊字符")

    # VITS配置
    vits_enabled: bool = Field(default=False, description="是否启用VITS")
    vits_url: str = Field(default="http://127.0.0.1:7860", description="VITS服务地址")
    vits_voice_id: int = Field(default=0, description="VITS说话人ID")
    vits_noise_scale: float = Field(default=0.667, description="VITS噪声缩放")
    vits_noise_scale_w: float = Field(default=0.8, description="VITS噪声缩放w")
    vits_length_scale: float = Field(default=1.0, ge=0.1, le=2.0, description="VITS长度缩放")

    # Genie-TTS配置
    genie_tts_enabled: bool = Field(default=False, description="是否启用Genie-TTS")
    genie_tts_url: str = Field(default="http://127.0.0.1:8000", description="Genie-TTS服务地址")
    genie_tts_speed: float = Field(default=1.0, ge=0.1, le=3.0, description="Genie-TTS语速")
    genie_tts_top_k: int = Field(default=15, ge=1, le=50, description="Genie-TTS top_k")
    genie_tts_top_p: float = Field(default=1.0, ge=0.0, le=1.0, description="Genie-TTS top_p")
    genie_tts_temperature: float = Field(default=1.0, ge=0.1, le=2.0, description="Genie-TTS temperature")
    genie_tts_ref_free: bool = Field(default=True, description="Genie-TTS是否免参考(True=API模式, False=本地模型模式)")
    genie_tts_ref_text: str = Field(default="", description="Genie-TTS参考文本")
    genie_tts_ref_audio_path: str = Field(default="", description="Genie-TTS参考音频路径")
    genie_tts_timeout: int = Field(default=60, ge=5, le=300, description="Genie-TTS请求超时时间(秒)")
...
class OnlineAIDrawConfig(BaseModel):
    """云端AI绘图配置"""
    api_url: str = Field(default="https://api.xingzhige.com/API/DrawOne/", description="云端绘图API地址")
    api_key: str = Field(default="", description="API密钥（如需要）")
    default_model: str = Field(default="anything-v5", description="默认绘图模型")
    default_size: str = Field(default="1:1", description="默认图片比例（1:1, 16:9, 4:3等）或尺寸（1024*1024）")
    provider: str = Field(default="auto", description="API提供商: auto（自动检测）, zhipu（智谱GLM）, generic（通用）")
    timeout: int = Field(default=60, ge=5, le=300, description="请求超时时间（秒）")

class LocalAIDrawConfig(BaseModel):
    """本地AI绘图配置"""
    service_url: str = Field(default="http://127.0.0.1:7860", description="本地绘图服务地址")
    service_type: str = Field(default="sd_webui", description="服务类型: sd_webui, comfyui, fooocus")
    model: str = Field(default="", description="指定模型名称（可选）")
    width: int = Field(default=512, ge=256, le=2048, description="图片宽度")
    height: int = Field(default=512, ge=256, le=2048, description="图片高度")
    steps: int = Field(default=20, ge=10, le=150, description="生成步数")
    cfg_scale: float = Field(default=7.0, ge=1.0, le=30.0, description="CFG缩放系数")
    sampler: str = Field(default="DPM++ 2M Karras", description="采样器名称")
    timeout: int = Field(default=300, ge=30, le=600, description="请求超时时间（秒）")
...
class VCPConfig(BaseModel):
    """VCPToolBox记忆系统配置"""
    enabled: bool = Field(default=False, description="是否启用VCP记忆系统")
    base_url: str = Field(default="http://127.0.0.1:6005", description="VCP服务器地址")
    key: str = Field(default="", description="VCP访问密钥")
    auto_sync: bool = Field(default=True, description="是否自动同步记忆")
    priority: int = Field(default=70, ge=0, le=100, description="VCP记忆检索优先级")

class BaodouAIConfig(BaseModel):
    """包豆AI视觉自动化配置"""
    enabled: bool = Field(default=False, description="是否启用包豆AI视觉自动化")
    config_path: str = Field(
        default="baodou_AI/config.json",
        description="包豆AI配置文件路径"
    )
    api_key: str = Field(default="", description="豆包API密钥")
    base_url: str = Field(
        default="https://ark.cn-beijing.volces.com/api/v3",
        description="豆包API基础地址"
    )
    model_name: str = Field(
        default="doubao-seed-1-6-vision-250815",
        description="视觉模型名称"
    )
    max_iterations: int = Field(default=80, ge=10, le=200, description="最大迭代次数")
    thinking_enabled: bool = Field(default=False, description="是否启用AI思考模式")
    auto_capture: bool = Field(default=True, description="是否自动截屏")
    optimize_for_speed: bool = Field(default=True, description="是否优化速度")
    safe_mode: bool = Field(default=True, description="是否启用安全模式")

class ToolPriorityConfig(BaseModel):
    """工具优先级配置"""
    # 是否启用工具优先级系统
    enabled: bool = Field(default=True, description="是否启用工具优先级系统")

    # 工具能力类别优先级（数值越高优先级越高）
    capability_priority: Dict[str, int] = Field(
        default={
            "screen_control": 100,      # 屏幕控制（包豆AI等）
            "browser_automation": 90,   # 浏览器自动化
            "web_search": 80,           # 网页搜索
            "app_launcher": 70,        # 应用启动
            "system_control": 60,      # 系统控制
            "file_operations": 50,      # 文件操作
            "data_retrieval": 40,       # 数据检索
            "messaging": 30,            # 消息发送
            "other": 10,                # 其他工具
        },
        description="工具能力类别优先级"
    )

    # 同类工具的降级策略
    fallback_strategy: str = Field(default="auto", description="降级策略: auto(自动), sequential(顺序), none(不降级)")

    # 超时配置（毫秒）
    tool_timeout: int = Field(default=30000, ge=5000, le=120000, description="单个工具调用超时时间")

    # 最大重试次数
    max_retries: int = Field(default=1, ge=0, le=5, description="工具调用失败后的最大重试次数")

    # 是否记录降级日志
    log_fallback: bool = Field(default=True, description="是否记录工具降级日志")

    # 工具白名单（优先使用这些工具）
    preferred_tools: List[str] = Field(
        default=["包豆AI视觉自动化"],
        description="优先使用的工具列表"
    )
...
class PromptManager:
    ...
    def get_prompt(self, name: str, **kwargs) -> str:
        ...
        try:
            ...
            # 格式化模板 - 使用安全的方法避免误解析JSON示例中的大括号
            if kwargs:
                try:
                    # 提取所有占位符
                    import re
                    # 只替换明确的占位符 {variable}，忽略JSON中的 {key: "value"}
                    # 使用正则匹配简单的占位符格式
                    for key, value in kwargs.items():
                        # 匹配 {key} 格式，其中key只包含字母、数字和下划线
                        pattern = r'\{' + re.escape(key) + r'\}'
                        content = re.sub(pattern, str(value), content)
                    return content
                except Exception as e:
                    print(f"错误：提示词 '{name}' 格式化失败: {e}")
                    return content
            else:
                return content
...
class LocationConfig(BaseModel):
    """地理位置感知配置"""
    enabled: bool = Field(default=True, description="是否启用地理位置感知")
    auto_detect: bool = Field(default=False, description="是否使用IP自动检测位置")
    manual_city: str = Field(default="", description="手动配置的城市名称（如：中国 贵州 黔南布依族苗族自治州）")
...
class ActiveCommunicationGeneratorConfig(BaseModel):
    """主动交流生成器配置"""
    temperature: float = Field(default=0.8, ge=0.0, le=2.0, description="对话生成温度")
    max_tokens: int = Field(default=150, ge=50, le=300, description="最大生成token数")
    use_memory: bool = Field(default=True, description="是否使用记忆系统")
    use_weather: bool = Field(default=True, description="是否使用天气信息")
    use_time_context: bool = Field(default=True, description="是否使用时间情境")


class ActiveCommunicationRegulatorConfig(BaseModel):
    """主动交流频率调节器配置"""
    base_interval: int = Field(default=30, ge=5, le=120, description="基础触发间隔(分钟)")
    min_interval: int = Field(default=10, ge=5, le=30, description="最小触发间隔(分钟)")
    max_interval: int = Field(default=120, ge=60, le=240, description="最大触发间隔(分钟)")
    adjustment_factor: float = Field(default=0.2, ge=0.1, le=0.5, description="频率调整因子")
    response_window: int = Field(default=600, ge=300, le=1200, description="响应时间窗口(秒)")


class ActiveCommunicationConfig(BaseModel):
    """主动交流系统配置"""
    enabled: bool = Field(default=False, description="是否启用主动交流系统")
    context_aware: bool = Field(default=False, description="是否启用情境感知")
    generator: ActiveCommunicationGeneratorConfig = Field(default_factory=ActiveCommunicationGeneratorConfig)
    regulator: ActiveCommunicationRegulatorConfig = Field(default_factory=ActiveCommunicationRegulatorConfig)
...
class NagaConfig(BaseModel):
    ...
    online_ai_draw: OnlineAIDrawConfig = Field(default_factory=OnlineAIDrawConfig)
    local_ai_draw: LocalAIDrawConfig = Field(default_factory=LocalAIDrawConfig)
    online_search: OnlineSearchConfig = Field(default_factory=OnlineSearchConfig)
    vcp: VCPConfig = Field(default_factory=VCPConfig)
    baodou_ai: BaodouAIConfig = Field(default_factory=BaodouAIConfig)
    active_communication: ActiveCommunicationConfig = Field(default_factory=ActiveCommunicationConfig)  # 主动交流系统配置
    ...
    tool_priority: ToolPriorityConfig = Field(default_factory=ToolPriorityConfig)  # 工具优先级配置
