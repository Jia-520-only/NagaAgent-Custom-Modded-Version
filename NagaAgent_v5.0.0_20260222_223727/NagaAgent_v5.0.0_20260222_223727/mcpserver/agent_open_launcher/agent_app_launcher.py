    ...
    async def handle_handoff(self, data: dict) -> str:
        ...
        try:
            ...
            if tool_name == "获取应用列表":
                ...
                # 启动应用工具
                app = data.get("app") or data.get("app_name") or data.get("param_name")
                args = data.get("args")
    ...
    async def _open_app(self, app_name: str, args: str = None) -> dict:
        ...
        try:
            ...
            if not app_info:
                ...
                print(f"⚠️ 未找到应用 '{app_name}'，正在获取可用应用列表...")

                # 只从缓存中获取已扫描的应用，不触发新的扫描
                try:
                    # 先尝试直接获取缓存（不触发扫描）
                    if self.scanner._scan_completed and self.scanner.apps_cache:
                        # 使用缓存的列表
                        available_apps = [{"name": app["name"], "path": app["path"]} for app in self.scanner.apps_cache[:20]]
                        print(f"✅ 从缓存获取 {len(available_apps)} 个应用")
                    else:
                        # 需要扫描时才扫描
                        app_info = await self.scanner.get_app_info_for_llm()
                        available_apps = app_info["apps"][:20]  # 只显示前20个 #
                except Exception as e:
                    print(f"⚠️ 获取应用列表失败: {e}")
                    available_apps = []
